...
const TESSDATA_FILE_URL = `file://${TESSDATA_DIR}`;
...
function ensureLanguageFiles() {
  fs.mkdirSync(TESSDATA_DIR, { recursive: true });
  const missing = languageList.filter(lang => !fs.existsSync(path.join(TESSDATA_DIR, `${lang}.traineddata`)));
  if (missing.length) {
    console.error(`❌ Missing language data: ${missing.join(', ')}`);
    console.error(`Place *.traineddata files in: ${TESSDATA_DIR}`);
    process.exit(1);
  }
}
...
async function recognize() {
  try {
    ...
    const result = await Tesseract.recognize(resolvedPath, languages, {
      langPath: TESSDATA_FILE_URL,
      gzip: false,
      logger: m => {
    ...
    if (outputJson) {
      console.log(JSON.stringify({
        ...
        words: normalizeWords(result?.data)
      }, null, 2));
    } else {
