...
class ProLinkMonitorGUI:
    ...
    def capture_video_cover(self, page, output_dir, link_index):
        ...
        try:
            ...
            if container:
                try:
                    cover_name = f'cover_{link_index}_video_screenshot.png'
    ...
    def step_through_carousel(self, page, output_dir, link_index, cover_paths, seen_urls, limit=10):
                ...
                for i, b in enumerate(bullets[:limit]):
                    ...
                    if src:
                        ...
                        if normalized and normalized not in seen_urls and not self.is_unwanted_image_src(normalized):
                            cover_name = f'cover_{link_index}_{len(cover_paths)+1}.png'
                            cover_path = os.path.join(output_dir, cover_name)
    ...
    def capture_cover_images(self, page, output_dir, link_index, image_urls=None):
        ...
        try:
            ...
            if is_video and len(cover_paths) < 1:
                ...
                video_covers = self.capture_video_cover(page, output_dir, link_index)
                if video_covers:
            ...
            if len(cover_paths) < 2 and not is_video:
                self.log("    [封面] 尝试轮播逐帧...")
                try:
                    self.step_through_carousel(page, output_dir, link_index, cover_paths, seen_urls if 'seen_urls' in locals() else set(), limit=6)
                except Exception as e:
    ...
    def capture_with_playwright_pro(self, links, screenshot_dir):
        """Pro版本的抓取功能"""
        results = []
        ...
        with sync_playwright() as p:
            ...
            for idx, link in enumerate(links, 1):
                ...
                self.update_progress(idx, len(links), f"处理链接 {idx}")
                self.log(f"[{idx}/{len(links)}] {link[:50]}...")
                ...
                try:
                    ...
                    if is_full_mode:
                        ...
                        # 抓取封面图
                        cover_paths = self.capture_cover_images(page, covers_dir, idx, image_urls)
    ...
    def create_excel_report_pro(self, results, output_dir):
        ...
        # 表头
        headers = [
            '序号', '链接', '采集状态', '错误信息', '笔记ID', '标题', '作者昵称', '作者ID', 
        ...
        for result in results:
            ...
            # 1. 序号
            cell = ws.cell(row=current_row, column=1, value=result['序号'])
            cell.alignment = Alignment(horizontal='center', vertical='center')
            cell.border = thin_border
            
            # 2. 链接
            cell = ws.cell(row=current_row, column=2, value=result['链接'])
            cell.alignment = Alignment(horizontal='left', vertical='center', wrap_text=True)
            cell.border = thin_border
            
            # 3. 采集状态
            status = result.get('采集状态', '失败')
            cell = ws.cell(row=current_row, column=3, value=status)
            if status == '成功':
            ...
            # 4. 错误信息
            cell = ws.cell(row=current_row, column=4, value=result.get('错误信息', ''))
            cell.alignment = Alignment(horizontal='left', vertical='center', wrap_text=True)
            cell.font = Font(color='DC3545', size=9)
            cell.border = thin_border
            
            # 5. 笔记ID
            cell = ws.cell(row=current_row, column=5, value=result.get('笔记ID', ''))
            cell.alignment = Alignment(horizontal='center', vertical='center')
            cell.font = Font(name='Consolas', size=9)
            cell.border = thin_border
            
            # 6. 标题
            cell = ws.cell(row=current_row, column=6, value=result.get('标题', ''))
            cell.alignment = Alignment(horizontal='left', vertical='center', wrap_text=True)
            cell.font = Font(bold=True, size=9)
            cell.border = thin_border
            
            # 7. 作者昵称
            cell = ws.cell(row=current_row, column=7, value=result.get('作者昵称', ''))
            cell.alignment = Alignment(horizontal='center', vertical='center')
            cell.font = Font(size=9)
            cell.border = thin_border
            
            # 8. 作者ID
            cell = ws.cell(row=current_row, column=8, value=result.get('作者ID', ''))
            cell.alignment = Alignment(horizontal='center', vertical='center')
            cell.font = Font(name='Consolas', size=9)
            cell.border = thin_border
            
            # 9. 粉丝数
            cell = ws.cell(row=current_row, column=9, value=result.get('粉丝数', ''))
            cell.alignment = Alignment(horizontal='center', vertical='center')
            cell.font = Font(bold=True, color='FF6347', size=9)
            cell.border = thin_border
            
            # 10. 博主主页
            cell = ws.cell(row=current_row, column=10, value=result.get('博主主页', ''))
            cell.alignment = Alignment(horizontal='left', vertical='center', wrap_text=True)
            cell.font = Font(size=8, color='0066CC', underline='single')
            cell.border = thin_border
            
            # 11. 发布时间
            cell = ws.cell(row=current_row, column=11, value=result.get('发布时间', ''))
            cell.alignment = Alignment(horizontal='center', vertical='center')
            cell.font = Font(size=9)
            cell.border = thin_border
            
            # 12. 点赞数
            cell = ws.cell(row=current_row, column=12, value=result.get('点赞数', ''))
            cell.alignment = Alignment(horizontal='center', vertical='center')
            cell.font = Font(size=9)
            cell.border = thin_border
            
            # 13. 收藏数
            cell = ws.cell(row=current_row, column=13, value=result.get('收藏数', ''))
            cell.alignment = Alignment(horizontal='center', vertical='center')
            cell.font = Font(size=9)
            cell.border = thin_border
            
            # 14. 评论数
            cell = ws.cell(row=current_row, column=14, value=result.get('评论数', ''))
            cell.alignment = Alignment(horizontal='center', vertical='center')
            cell.font = Font(size=9)
            cell.border = thin_border
            
            # 15. 分享数
            cell = ws.cell(row=current_row, column=15, value=result.get('分享数', ''))
            cell.alignment = Alignment(horizontal='center', vertical='center')
            cell.font = Font(size=9)
            cell.border = thin_border
            
            # 16. 封面链接
            cell = ws.cell(row=current_row, column=16, value=result.get('封面链接', ''))
            cell.alignment = Alignment(horizontal='left', vertical='center', wrap_text=True)
            cell.font = Font(size=8, color='0066CC')
            cell.border = thin_border
            
            # 17. 视频链接
            cell = ws.cell(row=current_row, column=17, value=result.get('视频链接', ''))
            cell.alignment = Alignment(horizontal='left', vertical='center', wrap_text=True)
            cell.font = Font(size=8, color='0066CC')
            cell.border = thin_border
            
            # 18. 正文
            cell = ws.cell(row=current_row, column=18, value=result.get('正文', ''))
            ...
            for i in range(max_covers):
                ...
                if i < len(cover_paths) and os.path.exists(cover_paths[i]):
                    try:
                        ...
                        img.height = height
                        # T..AC
                        anchor_col = chr(ord('T') + i) if i < 7 else None
                        if anchor_col is None:
                            # beyond 'Z', use AA/AB/AC manually
                            anchor_cols = ['AA', 'AB', 'AC']
                            anchor_col = anchor_cols[i - 7]
    ...
    def monitor_task(self):
        try:
            ...
            self.log(f"[链接] 找到 {len(links)} 个")
            
            if not links:
                messagebox.showwarning("警告", "CSV中没有链接")
                return
            
            results = self.capture_with_playwright_pro(links, screenshot_dir)
