"""
WPP MD 小红书链接监测工具 - v4.0 飞书整合终极版
整合原程序所有功能 + 反爬优化 + 登录增强 + 飞书多维表格同步
"""
...
class FeishuBitableSync:
    ...
    RECORDS_URL_TEMPLATE = "https://open.feishu.cn/open-apis/bitable/v1/apps/{app_token}/tables/{table_id}/records"
    RECORDS_BATCH_CREATE_URL_TEMPLATE = "https://open.feishu.cn/open-apis/bitable/v1/apps/{app_token}/tables/{table_id}/records/batch_create"
    FIELDS_URL_TEMPLATE = "https://open.feishu.cn/open-apis/bitable/v1/apps/{app_token}/tables/{table_id}/fields"
    RECORD_DETAIL_URL_TEMPLATE = "https://open.feishu.cn/open-apis/bitable/v1/apps/{app_token}/tables/{table_id}/records/{record_id}"
    DRIVE_UPLOAD_URL = "https://open.feishu.cn/open-apis/drive/v1/medias/upload_all"

    def __init__(self, logger=None, config_path="feishu_config.json"):
        ...
        self.upload_cache = {}
        self.field_type_warnings = set()
        self.field_type_logged = False
        self.load_config()
        if not self.config.get("field_mapping"):
            self.config["field_mapping"] = self.default_field_mapping()
    ...
    def format_response_error(self, response):
    ...
    def ensure_ok_response(self, response, action_label):
        if response.status_code != 200:
            raise RuntimeError(f"{action_label}失败: {self.format_response_error(response)}")
        try:
            data = response.json()
        except Exception as e:
            raise RuntimeError(f"{action_label}失败: 响应解析失败: {e}")
        if data.get("code") != 0:
            request_id = data.get("request_id", "")
            msg = data.get("msg", "")
            detail = ""
            ...
            raise RuntimeError(
                f"{action_label}失败: code {data.get('code')} msg {msg} request_id {request_id}".strip()
            )
        return data

    def delete_record(self, record_id, token):
        if not record_id:
            return
        url = self.RECORD_DETAIL_URL_TEMPLATE.format(
        ...
        try:
            ...
            self.ensure_ok_response(response, "删除诊断记录")
        except Exception as e:
            self.log(f"删除诊断记录失败: {e}", "warning")

    def diagnose_field_validation(self, fields, token, field_types=None):
        if not fields:
            return
        self.log("开始字段校验诊断（会临时创建并删除测试记录）", "warning")
        url = self.RECORDS_BATCH_CREATE_URL_TEMPLATE.format(
            app_token=self.config.get("app_token"),
            table_id=self.config.get("table_id")
        )
        headers = {
        ...
        for field_name, value in fields.items():
            payload = {"records": [{"fields": {field_name: value}}]}
            try:
                response = requests.post(url, headers=headers, json=payload, timeout=20)
                data = self.ensure_ok_response(response, f"字段诊断[{field_name}]")
                record_id = ""
                records = data.get("data", {}).get("records") or []
                if records and isinstance(records, list):
                    record_id = records[0].get("record_id", "")
                if record_id:
                    self.delete_record(record_id, token)
                self.log(f"字段诊断通过: {field_name}", "success")
            except Exception as e:
                field_type = ""
                if field_types:
                    field_type = self.normalize_field_type(field_types.get(field_name, ""))
                value_preview = str(value)
                if len(value_preview) > 120:
                    value_preview = value_preview[:120] + "..."
                detail = f" 类型={field_type}" if field_type else ""
                self.log(f"字段诊断失败: {field_name}{detail} 值={value_preview} -> {e}", "error")
    ...
    def default_field_mapping(self):
        ...
        mapping = {field: field for field in base_fields}
        mapping.update(extra_fields)
        return mapping

    def normalize_field_type(self, raw_type):
        if raw_type is None:
            return ""
        if isinstance(raw_type, str):
            value = raw_type.strip().lower()
            if "数字" in value or "数值" in value:
                return "number"
            if "文本" in value or "文字" in value:
                return "text"
            if "超链接" in value or "链接" in value or "url" in value:
                return "link"
            ...
            if "图片" in value:
                return "image"
            if "number" in value or value in ("int", "float", "number"):
                return "number"
            ...
            if "image" in value:
                return "image"
            if "attach" in value or "file" in value:
                return "attachment"
            return value
    ...
    def get_table_field_types(self, token):
        if self.field_type_cache:
            return self.field_type_cache

        headers = {"Authorization": f"Bearer {token}"}
        url = self.FIELDS_URL_TEMPLATE.format(
            app_token=self.config.get("app_token"),
            table_id=self.config.get("table_id")
        )
        field_types = {}
        ...
        while True:
            ...
            data = self.ensure_ok_response(response, "读取表结构")
            items = data.get("data", {}).get("items", []) or []
            for item in items:
                name = item.get("field_name") or item.get("name") or item.get("field_name_en")
                raw_type = item.get("type") or item.get("field_type")
                if name:
                    field_types[name] = self.normalize_field_type(raw_type)
            page_token = data.get("data", {}).get("page_token")
            if not data.get("data", {}).get("has_more"):
                break
        self.field_type_cache = field_types
        if field_types and not self.field_type_logged:
            self.field_type_logged = True
            fields_preview = ", ".join(f"{name}({ftype})" for name, ftype in field_types.items())
            self.log(f"飞书表结构字段: {fields_preview}", "debug")
        return field_types

    def parse_number_value(self, value):
        if value is None:
            return None
        ...
        multiplier = 1
        if "万" in text_upper:
            text_upper = text_upper.replace("万", "")
            multiplier = 10000
        elif "W" in text_upper:
            text_upper = text_upper.replace("W", "")
            multiplier = 10000
        elif "K" in text_upper:
            text_upper = text_upper.replace("K", "")
            multiplier = 1000
        text_upper = text_upper.replace(",", "")
        try:
            if "." in text_upper:
                return float(text_upper) * multiplier
            return int(text_upper) * multiplier
        except ValueError:
            digits = re.sub(r"[^\d.]", "", text_upper)
            try:
                if "." in digits:
    ...
    def parse_datetime_value(self, value):
        ...
        if digits.isdigit() and len(digits) >= 10:
            ...
            return num
        try:
            dt = datetime.fromisoformat(text.replace("Z", "").replace("/", "-"))
    ...
    def resolve_result_value(self, result, result_key):
        if result_key == "截图":
            return result.get("截屏文件", "")
        if result_key in result and result[result_key] not in (None, "", []):
            return result.get(result_key)
        match = re.match(r"封面(\d+)", result_key)
        if match:
            index = int(match.group(1)) - 1
            covers = result.get("封面图列表") or []
            if 0 <= index < len(covers):
                return covers[index]
        return result.get(result_key)

    def guess_field_type(self, result_key, field_name):
        number_keys = {"粉丝数", "点赞数", "收藏数", "评论数", "分享数"}
        link_keys = {"链接", "博主主页", "封面链接", "视频链接"}
        attachment_keys = {"截屏文件", "封面图列表", "截图"}
        if result_key in number_keys or field_name in number_keys:
            return "number"
        if result_key in link_keys or field_name in link_keys:
            return "link"
        if result_key in attachment_keys or field_name in attachment_keys:
            return "attachment"
        if result_key.startswith("封面") or field_name.startswith("封面"):
            return "attachment"
        return ""

    def upload_file(self, file_path, token, field_type="attachment"):
        if not file_path:
            return ""
        if file_path in self.upload_cache:
            return self.upload_cache[file_path]
        if not os.path.exists(file_path):
            self.log(f"附件不存在，跳过上传: {file_path}", "warning")
            return ""
        file_name = os.path.basename(file_path)
        file_size = os.path.getsize(file_path)
        parent_type = "bitable_image" if field_type == "image" else "bitable_file"
        parent_node = self.config.get("app_token") or ""
        if not parent_node:
            self.log("附件上传失败: 缺少 app_token", "warning")
            return ""
        headers = {"Authorization": f"Bearer {token}"}
        try:
            with open(file_path, "rb") as f:
                ...
                response = requests.post(self.DRIVE_UPLOAD_URL, headers=headers, data=data, files=files, timeout=30)
                payload = self.ensure_ok_response(response, "附件上传")
            token_value = (payload.get("data") or {}).get("file_token") or ""
            if token_value:
                self.upload_cache[file_path] = token_value
                return token_value
        except Exception as e:
            self.log(f"附件上传失败: {e}", "warning")
        return ""

    def build_attachment_field(self, value, token, field_type):
        if not value:
            return None
        paths = []
        if isinstance(value, (list, tuple)):
            paths = [item for item in value if item]
        else:
            paths = [value]
        attachments = []
        for path in paths[:10]:
            if isinstance(path, str) and path.startswith("http"):
                self.log(f"附件为URL，暂不支持直传: {path}", "warning")
                continue
            file_token = self.upload_file(str(path), token, field_type=field_type)
            if file_token:
    ...
    def load_config(self):
        try:
            ...
            self.log(f"读取飞书配置失败: {e}", 'warning')

    def save_config(self):
        try:
            with open(self.config_path, 'w', encoding='utf-8') as f:
                json.dump(self.config, f, ensure_ascii=False, indent=2)
            self.log("飞书配置已保存", 'success')
        except Exception as e:
            self.log(f"保存飞书配置失败: {e}", 'error')

    def update_config(self, data, persist=False):
        if not data:
            return
    ...
    def get_tenant_access_token(self):
        now = time.time()
        if self.tenant_access_token and now < self.token_expire_at - 60:
            return self.tenant_access_token

        if not self.is_ready():
            raise ValueError("飞书配置不完整，无法获取token")
        ...
        data = self.ensure_ok_response(response, "获取token")
        self.tenant_access_token = data.get("tenant_access_token")
        expire = data.get("expire", 0)
        self.token_expire_at = now + int(expire)
        return self.tenant_access_token

    def build_record_fields(self, result, field_types=None, token=None):
        fields = {}
        mapping = self.default_field_mapping()
        custom_mapping = self.config.get("field_mapping") or {}
        if isinstance(custom_mapping, dict):
            mapping.update(custom_mapping)
        type_overrides = self.config.get("field_types") or {}
        for result_key, feishu_field in mapping.items():
            if not feishu_field:
                continue

            if isinstance(feishu_field, dict):
                field_name = feishu_field.get("name", "").strip()
                field_type = feishu_field.get("type", "").strip().lower()
            else:
                field_name = str(feishu_field).strip()
                field_type = ""

            if not field_name:
                continue

            override_type = type_overrides.get(field_name) or type_overrides.get(result_key) or ""
            field_type = self.normalize_field_type(override_type) if override_type else ""
            if field_types:
                if field_name not in field_types:
                    continue
                server_type = self.normalize_field_type(field_types.get(field_name, ""))
                if server_type:
                    if field_type and field_type != server_type:
                        warn_key = (field_name, field_type, server_type)
                        if warn_key not in self.field_type_warnings:
                            self.field_type_warnings.add(warn_key)
                            self.log(f"字段类型冲突: {field_name} 配置={field_type} 表结构={server_type}", "warning")
                    field_type = server_type
            if not field_type:
                field_type = self.guess_field_type(result_key, field_name)

            value = self.resolve_result_value(result, result_key)
            if value is None or value == "":
                continue

            if field_type in ("number", "int", "float"):
                number_value = self.parse_number_value(value)
                if number_value is None:
                    continue
                fields[field_name] = number_value
                continue

            if field_type in ("attachment", "image", "file"):
                if not token:
                    continue
                attachments = self.build_attachment_field(value, token, field_type)
                if attachments:
                    fields[field_name] = attachments
                continue

            if field_type in ("link", "url"):
                if isinstance(value, dict):
                    link_value = value
                else:
                    link_text = str(value).strip()
                    if not link_text:
                        continue
                    link_value = {"text": link_text, "link": link_text}
                fields[field_name] = link_value
                continue

            if field_type in ("datetime", "date", "time"):
                datetime_value = self.parse_datetime_value(value)
                if datetime_value is None:
                    continue
                fields[field_name] = datetime_value
                continue

            if isinstance(value, (list, tuple)):
                sanitized = "\n".join(str(item) for item in value if item)
            else:
                sanitized = value if isinstance(value, (int, float)) else str(value)
            fields[field_name] = sanitized
        return fields

    def chunk_records(self, items, size=50):
        for i in range(0, len(items), size):
            yield items[i:i + size]

    def sync_results(self, results):
        if not self.config.get("enabled"):
            return False
        if not results:
            self.log("没有可同步到飞书的数据", 'warning')
            return False
        if not self.is_ready():
            raise ValueError("飞书配置不完整，请检查 AppID/AppSecret 等信息")
        ...
        field_types = self.get_table_field_types(token)

        total_synced = 0
        for chunk in self.chunk_records(results):
            records_payload = []
            for result in chunk:
                fields = self.build_record_fields(result, field_types=field_types, token=token)
                if fields:
            ...
            try:
                self.ensure_ok_response(response, "同步飞书")
            except Exception as e:
                error_text = str(e)
                if records_payload:
                    sample_fields = records_payload[0].get("fields", {})
                    if isinstance(sample_fields, dict):
                        field_names = ", ".join(sample_fields.keys())
                        self.log(f"飞书字段提交: {field_names}", "warning")
                        if "field validation failed" in error_text:
                            self.diagnose_field_validation(sample_fields, token, field_types=field_types)
                if "99991672" in error_text or "Access denied" in error_text:
                    self.log("飞书权限不足: 请在应用权限中开通 bitable:app 或 base:record:create", "error")
                raise
            total_synced += len(records_payload)
            time.sleep(0.5)  # 简单限流

        self.log(f"飞书多维表格同步完成：{total_synced} 条记录", 'success')
        return True
    ...
    def test_connection(self):
        ...
        params = {"page_size": 1}
        response = requests.get(url, headers=headers, params=params, timeout=15)
        self.ensure_ok_response(response, "读取表格")
        return True
...
class UltimateMonitorV3GUI:
    def __init__(self, root):
        self.root = root
        self.root.title("WPP MD 小红书链接监测工具 v4.0 Ultimate")
        self.root.geometry("1100x850")
        self.version_tag = "v4.0"
        ...
        # v4.0 飞书集成
        self.feishu_config_path = "feishu_config.json"
        self.feishu_sync = FeishuBitableSync(logger=self.log, config_path=self.feishu_config_path)
        feishu_cfg = self.feishu_sync.config
        self.enable_feishu_sync = tk.BooleanVar(value=feishu_cfg.get("enabled", False))
        self.feishu_app_id = tk.StringVar(value=feishu_cfg.get("app_id", ""))
        self.feishu_app_secret = tk.StringVar(value=feishu_cfg.get("app_secret", ""))
        self.feishu_app_token = tk.StringVar(value=feishu_cfg.get("app_token", ""))
        self.feishu_table_id = tk.StringVar(value=feishu_cfg.get("table_id", ""))

        # 日志文件配置
        self.log_dir = os.path.join(os.getcwd(), "logs_v4")
    ...
    def create_ultimate_widgets(self):
        ...
        title_label = tk.Label(
            title_container,
            text=f"WPP MD 小红书链接监测工具 {self.version_tag}",
        ...
        subtitle_label = tk.Label(
            title_container,
            text="Ultimate Anti-Detection Version | 智能反爬监测 | 飞书多维表格同步",
        ...
        content_canvas = tk.Canvas(content_wrapper, bg=self.colors['bg'], highlightthickness=0)
        content_scrollbar = ttk.Scrollbar(content_wrapper, orient="vertical", command=content_canvas.yview)
        content_canvas.configure(yscrollcommand=content_scrollbar.set)
        content_canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        content_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)

        scroll_frame = tk.Frame(content_canvas, bg=self.colors['bg'])
        scroll_window = content_canvas.create_window((0, 0), window=scroll_frame, anchor="nw")

        def _on_frame_configure(event):
            content_canvas.configure(scrollregion=content_canvas.bbox("all"))

        def _on_canvas_configure(event):
            content_canvas.itemconfig(scroll_window, width=event.width)
        ...
        content_canvas.bind_all("<Button-4>", lambda e: content_canvas.yview_scroll(-1, "units"))
        content_canvas.bind_all("<Button-5>", lambda e: content_canvas.yview_scroll(1, "units"))

        main_container = tk.Frame(scroll_frame, bg=self.colors['bg'])
        ...
        anti_note = tk.Label(anti_detection_frame, text=f"({self.version_tag} 反爬增强)", font=('Microsoft YaHei UI', 8),
                            bg=self.colors['card_bg'], fg=self.colors['text_secondary'])
        anti_note.pack(side=tk.LEFT, padx=(5, 0))

        # ===== 飞书同步配置 =====
        feishu_card = self.create_ultimate_card(main_container, "☁️ 飞书多维表格同步 (v4.0核心)")
        feishu_card.pack(fill=tk.X, pady=(0, 15))

        feishu_top = tk.Frame(feishu_card, bg=self.colors['card_bg'])
        feishu_top.pack(fill=tk.X, pady=(0, 10))

        self.feishu_toggle = tk.Checkbutton(
            feishu_top,
            text="启用飞书多维表格实时同步",
            variable=self.enable_feishu_sync,
            font=('Microsoft YaHei UI', 10, 'bold'),
            bg=self.colors['card_bg'],
            fg=self.colors['text_primary'],
            activebackground=self.colors['card_bg'],
            selectcolor=self.colors['card_bg'],
            command=self.on_feishu_toggle
        )
        self.feishu_toggle.pack(side=tk.LEFT)

        feishu_hint = tk.Label(
            feishu_top,
            text="配置 AppID/AppSecret 及多维表格 app_token、table_id 后即可同步",
            font=('Microsoft YaHei UI', 8),
            bg=self.colors['card_bg'],
            fg=self.colors['text_secondary']
        )
        feishu_hint.pack(side=tk.LEFT, padx=(10, 0))

        feishu_form = tk.Frame(feishu_card, bg=self.colors['card_bg'])
        feishu_form.pack(fill=tk.X, pady=5)

        field_specs = [
            ("App ID", self.feishu_app_id, False),
            ("App Secret", self.feishu_app_secret, True),
            ("多维表格 App Token", self.feishu_app_token, False),
            ("多维表格 Table ID", self.feishu_table_id, False),
        ]

        for idx, (label_text, var, is_secret) in enumerate(field_specs):
            frame = tk.Frame(feishu_form, bg=self.colors['card_bg'])
            frame.pack(fill=tk.X, pady=4)
            lbl = tk.Label(frame, text=label_text + ":", font=('Microsoft YaHei UI', 9, 'bold'),
                           bg=self.colors['card_bg'], fg=self.colors['text_primary'])
            lbl.pack(side=tk.LEFT, padx=(0, 10))
            entry = tk.Entry(frame, textvariable=var, font=('Consolas', 10),
                             bg=self.colors['bg'], fg=self.colors['text_primary'],
                             relief=tk.FLAT, show="*" if is_secret else "")
            entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 5))

        feishu_actions = tk.Frame(feishu_card, bg=self.colors['card_bg'])
        feishu_actions.pack(fill=tk.X, pady=(8, 0))

        tk.Button(
            feishu_actions,
            text="保存配置",
            command=self.save_feishu_config,
            font=('Microsoft YaHei UI', 9, 'bold'),
            bg=self.colors['primary'],
            fg=self.colors['card_bg'],
            relief=tk.FLAT,
        ...
        self.feishu_status_label = tk.Label(
            feishu_card,
            text="飞书同步未开启",
        ...
        self.feishu_status_label.pack(anchor=tk.W, pady=(6, 0))
        self.on_feishu_toggle()
        ...
        self.progress_label = tk.Label(progress_inner, text=f"就绪 - {self.version_tag} Ultimate 已就绪",
        ...
        tk.Button(log_toolbar, text="打开日志目录", command=self.open_log_folder,
                 font=('Microsoft YaHei UI', 8), bg=self.colors['bg'], fg=self.colors['text_primary'],
                 relief=tk.FLAT, padx=10, pady=3, cursor='hand2').pack(side=tk.RIGHT, padx=(5, 0))
        ...
        self.status_bar = tk.Label(status_frame, text=f"● {self.version_tag} Ultimate 就绪 | 飞书同步随时可用",
    ...
    def create_ultimate_card(self, parent, title):
        ...
        # 版本标识
        version_label = tk.Label(title_frame, text=self.version_tag, font=('Microsoft YaHei UI', 8, 'bold'),
    ...
    def get_feishu_config_from_ui(self):
        return {
            "enabled": self.enable_feishu_sync.get(),
            "app_id": self.feishu_app_id.get().strip(),
            "app_secret": self.feishu_app_secret.get().strip(),
            "app_token": self.feishu_app_token.get().strip(),
            "table_id": self.feishu_table_id.get().strip(),
        }
    ...
    def create_new_log_file(self):
        try:
            ...
            self.log_file_path = os.path.join(self.log_dir, f"ultimate_v4_{timestamp}.log")
            with open(self.log_file_path, 'w', encoding='utf-8') as f:
                f.write(f"==== Ultimate v4 Monitor Log | {timestamp} ====\n")
        except Exception as e:
            print(f"创建日志文件失败: {e}")
            self.log_file_path = None
    ...
    def open_log_folder(self):
        try:
            ...
            messagebox.showerror("日志目录", f"无法打开日志目录:\n{e}")
            self.log(f"打开日志目录失败: {e}", 'error')

    def update_feishu_status(self, message, level='info'):
        if not hasattr(self, 'feishu_status_label'):
            return
        color_map = {
        ...
        color = color_map.get(level, self.colors['text_secondary'])
        self.feishu_status_label.config(text=message, fg=color)

    def on_feishu_toggle(self):
        if self.enable_feishu_sync.get():
            self.update_feishu_status("飞书同步已启用", 'success')
        else:
            self.update_feishu_status("飞书同步未开启", 'info')

    def save_feishu_config(self):
        config = self.get_feishu_config_from_ui()
        self.feishu_sync.update_config(config, persist=True)
        self.update_feishu_status("飞书配置已保存", 'success')
        messagebox.showinfo("飞书配置", "飞书多维表格配置保存成功。")

    def test_feishu_connection(self):
        config = self.get_feishu_config_from_ui()
        self.feishu_sync.update_config(config, persist=False)
        try:
            self.feishu_sync.test_connection()
            self.update_feishu_status("飞书连接成功，可正常同步", 'success')
            messagebox.showinfo("飞书配置", "连接飞书成功，API可用。")
        except Exception as e:
            self.update_feishu_status(f"测试失败: {e}", 'error')
            messagebox.showerror("飞书配置", f"连接飞书失败:\n{e}")
            self.log(f"飞书连接失败: {e}", 'error')

    def sync_results_to_feishu(self, results):
        config = self.get_feishu_config_from_ui()
        self.feishu_sync.update_config(config, persist=False)
        if not self.enable_feishu_sync.get():
            return False
        try:
            synced = self.feishu_sync.sync_results(results)
            if synced:
                self.update_feishu_status("飞书同步完成", 'success')
            return synced
        except Exception as e:
            self.update_feishu_status("飞书同步失败，请检查日志", 'error')
            self.log(f"飞书同步失败: {e}", 'error')
            return False
    ...
    def safe_page_eval(self, page, script, label="", retries=2, wait_ms=5000):
        """在页面可能跳转时安全执行 evaluate，避免上下文销毁错误"""
        last_error = None
        for attempt in range(retries + 1):
            try:
                ...
                last_error = e
                error_text = str(e)
                if "Execution context was destroyed" in error_text or "navigation" in error_text:
                    if attempt < retries:
                        self.log(f"  [页面] 页面跳转中，等待后重试{label}", 'warning')
                        time.sleep(1.0)
                        continue
                break
        self.log(f"  [页面] evaluate失败{label}: {last_error}", 'warning')
        return None
    ...
    def check_login_state_comprehensive(self, page):
        ...
        try:
            ...
            login_state = page.evaluate("""() => {
                ...
                // 综合判断登录状态
                checks.isLoggedIn = (
                    !checks.hasLoginPopup &&
                    !checks.isLoginPage &&
                    (checks.hasAvatar || checks.hasUserInfo || checks.hasWebSession || checks.hasXsecAppid)
                );
    ...
    def capture_with_ultimate_features(self, items, screenshot_dir):
        ...
        self.log(f"启动{self.version_tag} Ultimate浏览器引擎...", 'info')
        ...
        with sync_playwright() as p:
            ...
            for idx, item in enumerate(items, 1):
                ...
                try:
                    ...
                    for attempt in range(max_retries):
                        try:
                            ...
                            try:
                                ...
                                if current_url and current_url != link:
                                    if any(key in current_url for key in ("login", "passport", "verify", "captcha")):
                                        self.log(f"  [链接] 页面跳转到登录/验证页: {current_url}", 'warning')
                                    else:
                                        self.log(f"  [链接] 实际跳转URL: {current_url}", 'info')
                            except Exception:
                                pass
                            ...
                            if resp and resp.status == 200:
                                ...
                                time.sleep(3)
                                # 检查页面是否有内容
                                page_content = self.safe_page_eval(
                                    page,
                                    """() => {
                                ...
                                if page_content is None:
                                    self.log("  [链接] 页面内容检查失败，准备重试...", 'warning')
                                    continue
                                if page_content:
                    ...
                    # 检查登录弹窗和页面状态
                    try:
                        page_status = self.safe_page_eval(
                            page,
                            """() => {
                                return {
                                    hasLoginPopup: !!(document.querySelector('.login-modal, [class*="LoginModal"]') ||
                    ...
                    if is_full_mode:
                        ...
                        try:
                            ...
                            self.sync_cover_url_fields(result, image_urls)
                        ...
                        try:
                            ...
                            if not result.get('封面链接'):
                                fallback_cover_urls = []
                                try:
                                    self.ensure_media_loaded(page)
                                    fallback_cover_urls = self.extract_note_image_urls_from_dom(page, limit=1)
                                except Exception:
                                    fallback_cover_urls = []
                                self.sync_cover_url_fields(result, fallback_cover_urls)
    ...
    def sync_cover_url_fields(self, target, image_urls, max_covers=10):
        """同步封面链接字段（仅保留封面1 URL）"""
        ...
        for item in image_urls or []:
            if not item:
                continue
            text = str(item).strip()
            if not text or text in normalized_urls:
                continue
            normalized_urls.append(text)
            if len(normalized_urls) >= max_covers:
                break

        if normalized_urls:
            target["封面链接"] = normalized_urls[0]
        elif target.get("封面链接"):
            target["封面链接"] = str(target.get("封面链接", "")).strip()
    ...
    def extract_detailed_content_ultimate(self, page, url):
        ...
        try:
            ...
            time.sleep(1.2)
            try:
                page.wait_for_selector('a[href*="/user/profile/"]', timeout=3000)
            except Exception:
                pass
            ...
            page_data = page.evaluate(
                r"""(noteId) => {
                    ...
                    if (detail) {
                        ...
                        const videoCover = pick(
                            video.poster, video.poster_url, video.posterUrl, video.cover, video.cover_url, video.coverUrl,
                            video.first_frame, video.firstFrame, video.thumbnail, video.thumbnail_url, video.image
                        );
                        if (typeof videoCover === 'string' && videoCover) {
                            result.videoCoverUrl = videoCover;
                        } else if (videoCover && typeof videoCover === 'object') {
                            const nestedCover = pick(
                                videoCover.url, videoCover.url_default, videoCover.image_url, videoCover.poster, videoCover.src,
                                Array.isArray(videoCover.url_list) ? videoCover.url_list[0] : '',
                                Array.isArray(videoCover.urlList) ? videoCover.urlList[0] : ''
                            );
                            if (nestedCover) result.videoCoverUrl = nestedCover.toString();
                        }
                    }
            ...
            raw_cover_url = page_data.get("coverUrl") or page_data.get("videoCoverUrl") or ""
            details["封面链接"] = self.normalize_media_url(page.url, raw_cover_url) if raw_cover_url else ""
            ...
            if image_urls:
                ...
                try:
                    self.ensure_media_loaded(page)
                    dom_urls = self.extract_note_image_urls_from_dom(page, limit=10)
                except Exception:
                    dom_urls = []
                if dom_urls:
                    ordered = []
                    used = set()
                    for u in dom_urls:
                        if u in image_urls and u not in used:
                            ordered.append(u)
                            used.add(u)
                    for u in image_urls:
                        if u not in used:
            ...
            try:
                dom_data = page.evaluate(r"""() => {
                    ...
                    const poster = videoEl ? pickAttr(videoEl, 'poster', 'data-poster') : '';
                    const ogImageEl = document.querySelector('meta[property=\"og:image\"], meta[name=\"og:image\"], meta[name=\"twitter:image\"]');
                    const ogImage = ogImageEl ? (ogImageEl.getAttribute('content') || '') : '';
                    data.videoCoverUrl = poster || ogImage || '';
            ...
            if dom_data:
                ...
                if not details.get("封面链接"):
                    dom_cover_url = self.normalize_media_url(page.url, dom_data.get("coverUrl") or dom_data.get("videoCoverUrl"))
                    if dom_cover_url and not self.is_unwanted_image_src(dom_cover_url):
                        details["封面链接"] = dom_cover_url
            ...
            # 视频笔记兜底：若页面存在 video，但链接/封面仍为空，尝试从DOM与性能资源补齐
            media_fallback = self.extract_video_media_fallback(page)
            if media_fallback.get("has_video") and (not details.get("视频链接") or not details.get("封面链接")):
                if not details.get("视频链接") and media_fallback.get("video_url"):
                    details["视频链接"] = media_fallback.get("video_url", "")
                    self.log("  [视频] 已补全视频链接（DOM/资源记录）", "info")
                if not details.get("封面链接") and media_fallback.get("cover_url"):
                    details["封面链接"] = media_fallback.get("cover_url", "")
                    self.log("  [视频] 已补全封面链接（DOM/资源记录）", "info")
                if not details.get("视频链接") and not details.get("封面链接"):
                    self.log(
                        f"  [视频] 存在视频元素但未暴露可用直链（候选数:{media_fallback.get('candidate_count', 0)}，可能MSE/blob或反爬限制）",
                        "warning"
                    )

            if not details.get("作者ID") or not details.get("博主主页") or not details.get("作者昵称"):
                author_info = self.extract_author_info_from_dom(page)
                if author_info:
                    if not details.get("作者ID") and author_info.get("authorId"):
                        details["作者ID"] = author_info.get("authorId", "")
                        self.log(f"  [作者] DOM补全作者ID: {details['作者ID']}", 'info')
                    if not details.get("博主主页") and author_info.get("authorHome"):
                        details["博主主页"] = author_info.get("authorHome", "")
                        self.log(f"  [作者] DOM补全主页: {details['博主主页']}", 'info')
                    if not details.get("作者昵称") and author_info.get("authorName"):
                        details["作者昵称"] = author_info.get("authorName", "")
                        self.log(f"  [作者] DOM补全昵称: {details['作者昵称']}", 'info')
        ...
        self.sync_cover_url_fields(details, image_urls)
    ...
    def extract_video_media_fallback(self, page):
        """视频笔记URL兜底提取：DOM + meta + performance资源记录"""
        try:
            raw_data = page.evaluate(
                r"""() => {
                    ...
                    const videoEl = document.querySelector('video');
                    const sourceEl = videoEl ? videoEl.querySelector('source') : null;
                    const sourceNodes = Array.from(document.querySelectorAll('source[src]'));

                    const videoCandidates = uniq([
                        videoEl ? (videoEl.currentSrc || '') : '',
                        videoEl ? (videoEl.src || '') : '',
                        sourceEl ? (sourceEl.src || '') : '',
                        ...sourceNodes.map((s) => s.src || ''),
                    ]);
                    const coverCandidates = uniq([
                        videoEl ? (videoEl.getAttribute('poster') || videoEl.getAttribute('data-poster') || '') : '',
                        pickMeta('meta[property="og:image"]'),
                        pickMeta('meta[name="og:image"]'),
                        pickMeta('meta[name="twitter:image"]'),
                        ...Array.from(document.querySelectorAll('link[rel="preload"][as="image"]')).map((n) => n.href || ''),
                    ]);

                    const perfUrls = uniq(
                        (performance.getEntriesByType('resource') || [])
                            .map((entry) => (entry && entry.name) ? entry.name.toString() : '')
                            .filter(Boolean)
                    );
            ...
            video_candidates = raw_data.get("videoCandidates") or []
            cover_candidates = raw_data.get("coverCandidates") or []
            perf_urls = raw_data.get("perfUrls") or []

            normalized = []
            for value in list(video_candidates) + list(cover_candidates) + list(perf_urls):
                normalized_url = self.normalize_media_url(page.url, value)
                if not normalized_url or normalized_url in normalized:
                    continue
            ...
            for value in normalized:
                text = value.lower()
                if any(key in text for key in (".mp4", ".m3u8", "/video", "video_mp4", "h264")):
                    video_url = value
                    break
            ...
            for value in normalized:
                text = value.lower()
                if any(key in text for key in ("notes_pre_post", "!nd_", ".jpg", ".jpeg", ".png", ".webp", ".avif", "/image", "/img")):
                    cover_url = value
                    break

            return {
                "has_video": bool(raw_data.get("hasVideo")),
                "video_url": video_url,
                "cover_url": cover_url,
            ...
            raw_urls = page.evaluate(
                r"""(maxCount) => {
                    ...
                    if (carousel && carousel.el) {
                        ...
                        const slides = Array.from(carousel.el.querySelectorAll('.swiper-slide'));
                        const indexed = slides.map((slide) => {
                            const idxAttr = slide.getAttribute('data-swiper-slide-index');
                            const idx = idxAttr !== null ? parseInt(idxAttr, 10) : NaN;
                            const img = slide.querySelector('img');
                            return { idx, src: getImgSrc(img) };
                        }).filter(item => item.src);

                        if (indexed.some(item => !Number.isNaN(item.idx))) {
                            const byIndex = new Map();
                            for (const item of indexed) {
                                if (Number.isNaN(item.idx)) continue;
                                if (!byIndex.has(item.idx)) byIndex.set(item.idx, item.src);
                            }
                            urls = Array.from(byIndex.entries())
                                .sort((a, b) => a[0] - b[0])
                                .map(entry => entry[1]);
                        } else {
    ...
    def extract_author_info_from_dom(self, page):
        """从页面DOM补全作者信息（作者ID/主页/昵称）"""
        try:
            data = page.evaluate(
                r"""() => {
                    const result = { authorId: '', authorHome: '', authorName: '' };
                    const pickText = (el) => {
                        if (!el) return '';
                    ...
                    const pickFromLink = (link) => {
                        ...
                        if (match) {
                            ...
                            result.authorHome = href;
                        }
                        const name = pickText(link);
                        if (name) result.authorName = name;
                        return !!result.authorId;
                    };
                    const links = Array.from(document.querySelectorAll('a[href*="/user/profile/"]'));
                    const scored = links.map((a) => {
                        const rect = a.getBoundingClientRect();
                        const visible = rect.width > 5 && rect.height > 5 && a.offsetParent !== null;
                        return { a, rect, visible };
                    }).filter(item => item.visible && item.rect.top > -200 && item.rect.top < 1400);
                    scored.sort((a, b) => a.rect.top - b.rect.top);
                    for (const item of scored) {
                        if (pickFromLink(item.a)) break;
                    }

                    if (!result.authorId) {
                        const attrSelectors = ['[data-user-id]', '[data-userid]', '[data-uid]', '[data-user]'];
                        for (const sel of attrSelectors) {
                            const el = document.querySelector(sel);
                            if (!el) continue;
                            const attrs = ['data-user-id', 'data-userid', 'data-uid', 'data-user'];
                    ...
                    if (!result.authorId) {
                        ...
                        for (const s of scripts) {
                            ...
                            if (!text) continue;
                            const m = text.match(/user_id\"?\\s*:\\s*\"([a-zA-Z0-9]+)\"/);
                            if (m) {
                                result.authorId = m[1];
                                break;
                            }
                    ...
                    if (!result.authorName) {
                        const nameEl = document.querySelector('.author-name, .username, .user-nickname, [class*="author"][class*="name"], [class*="nickname"]');
                        result.authorName = pickText(nameEl);
                    }
                    if (!result.authorHome && result.authorId) {
                        result.authorHome = `https://www.xiaohongshu.com/user/profile/${result.authorId}`;
                    }
                    return result;
                }""",
            )
    ...
    def normalize_media_url(self, base_url, url):
        ...
        try:
            ...
            # 兼容异常拼接：两个URL直接粘连时，仅保留第一个URL
            http_matches = list(re.finditer(r"https?://", url))
            if len(http_matches) >= 2:
                second_start = http_matches[1].start()
                if second_start > 0:
                    url = url[:second_start].strip()
                    self.log("    检测到拼接URL，已截断为首个URL", "warning")
    ...
    def create_ultimate_excel_report(self, results, output_dir):
        ...
        ws = wb.active
        ws.title = f"小红书监测{self.version_tag}"
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        version_safe = self.version_tag.replace('.', '_')
        output_file = os.path.join(output_dir, f"小红书监测{version_safe}_Ultimate_{timestamp}.xlsx")
        wb.save(output_file)
    ...
    def monitor_task(self):
        ...
        try:
            ...
            if self.log_file_path:
                self.log(f"日志输出: {self.log_file_path}", 'info')

            self.log("="*60)
            self.log(f"{self.version_tag} Ultimate 监测任务开始", 'info')
            ...
            if self.enable_feishu_sync.get():
                self.log("飞书多维表格同步: 已启用", 'info')
            else:
                self.log("飞书多维表格同步: 未启用", 'info')
            self.log("="*60)
            ...
            if not self.stop_flag:
                self.log("生成最新 Excel 报告...", 'info')
                output_file, success, fail, anti_crawler = self.create_ultimate_excel_report(results, output_dir)

                feishu_synced = False
                if self.enable_feishu_sync.get():
                    self.log("同步数据到飞书多维表格...", 'info')
                    feishu_synced = self.sync_results_to_feishu(results)
                    if feishu_synced:
                        self.log("飞书多维表格同步完成", 'success')
                    else:
                        self.log("飞书多维表格同步失败或已跳过", 'warning')
                ...
                self.log("="*60)
                self.log(f"{self.version_tag} Ultimate 监测任务完成！", 'success')
                ...
                messagebox.showinfo(
                    f"✅ {self.version_tag} Ultimate 完成",
                    f"{self.version_tag} Ultimate 监测完成！\n\n"
                    f"总计: {len(results)} 条\n"
                    f"✅ 成功: {success} 条\n"
                    f"❌ 失败: {fail} 条\n"
                    f"🛡️ 反爬触发: {anti_crawler} 条\n"
                    f"☁️ 飞书同步: "
                    f"{'已完成' if feishu_synced else ('未启用' if not self.enable_feishu_sync.get() else '失败，请查看日志')}\n"
                    f"⏱️ 耗时: {int(minutes)}分{int(seconds)}秒\n\n"
                    f"{self.version_tag} 新特性:\n"
                    f"✅ 智能反爬检测\n"
                    f"✅ 随机时间间隔\n"
                    f"✅ 多重登录重试\n"
                    f"✅ 飞书多维表格同步\n\n"
            ...
            self.log(f"{self.version_tag} 任务执行失败: {str(e)}", 'error')
            messagebox.showerror("错误", f"{self.version_tag} 监测失败:\n{str(e)}")
    ...
    def stop_monitoring(self):
        if self.is_running:
            self.stop_flag = True
            self.log(f"正在停止{self.version_tag}监测任务...", 'warning')
